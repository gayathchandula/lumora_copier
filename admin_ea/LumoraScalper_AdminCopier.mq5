...
input double CurrentCandleBlend = 0.4;                   // Current Candle Blend Factor (0.0-1.0)
...
input double MaxSpreadPoints = 0.0;                         // Max Spread in Points (0 = Auto: ATR-based)
...
input double ZonePoints = 500.0;                            // Zone Points to Avoid Duplicate Signals
...
input double MinBreakEvenProfit = 0.4;                    // Min Profit to Trigger Break-Even ($ | 0 = Disabled)
...
input double MinBuySignalScore = 5.0;                     // Min Signal Strength Score to Buy (0.0 - 10.0)
input double MinSellSignalScore = 5.0;                    // Min Signal Strength Score to Sell (0.0 - 10.0)
...
input ENUM_LIMIT_ANCHOR LimitEntryAnchor = LIMIT_ANCHOR_SWING; // Pullback Anchor (Smart = nearer of Swing/EMA)
...
input int MaxLosingPositionsSameDir = 1;                  // Max Losing Positions in Same Direction Before Block
...
input int ConsecutiveLossesBeforeCooldown = 1;            // Consecutive Losses Before Cooldown Activates
...
input double MinHealthScore = 0.4;                       // Min Health Score to Hold Position (0.0 - 1.0)
input double MaxAdverseATR = 1.5;                         // Max Adverse Movement in ATR Multiples
input double HealthTrendWeight = 0.4;                    // Health Weight: Trend Alignment
...
input double HealthSwingWeight = 0.1;                    // Health Weight: Swing Level
...
input double PartialClose50Pct = 0.5;                    // Close % When Signal Drops to 50% of Initial
input double PartialClose25Pct = 1.0;                    // Close % When Signal Drops to 25% of Initial (Remaining)
...
input double SLTightenMinHealthPct = 0.5;                // Start Tightening Below This Health %
...
input bool EnableHedgeChain = false;                       // Enable Hedge Chain (MARTINGALE - high risk)
...
input double HedgeMaxLot = 0.1;                          // Hard Lot Ceiling Per Hedge Leg
...
input double MaxDrawdownFromPeak = 0.0;                     // Max Equity $ Drawdown - Pause Trading when Reached (0 = Disabled)
...
input double TargetEquity = 0.0;                            // Target Equity - Stop Trading when Reached (0 = Disabled)
input double MinimumEquity = 20.0;                          // Min Equity - Stop Trading when Reached (0 = Disabled)
...
input ENUM_INPUT_TYPE SLInputType = INPUT_DOLLAR;        // SL Input Type
input double SLValue = 5.0;                              // SL Value
...
input group "💥 Spike Exhaustion Block"
input bool EnableSpikeExhaustionBlock = true;             // Block Entry if Recent Big Candle + Very High Score
input int SpikeExhaustionLookbackCandles = 2;             // Check Last N Closed Candles For an Oversized One
input double SpikeExhaustionRangeATR = 1.6;               // Candle Range (x ATR) That Counts as "Big"
input double SpikeExhaustionMinScore = 6.0;               // Score Above Which This Block Applies

input group "🔗 Trade Copier (Admin)"
input bool EnableTradeCopier = false;                     // Broadcast Trades to the Copier Server
input string CopierServerURL = "";                        // Copier Server URL (e.g. https://your-domain.com)
input string CopierAdminKey = "";                         // Copier Server ADMIN_KEY
input double DailyProfitLimitToStopCopying = 0;           // Stop Broadcasting NEW Opens Once Today's Realized Profit Reaches This ($, 0 = Disabled)
...
// Trade Copier Tracking - today's realized profit, for DailyProfitLimitToStopCopying
double dailyRealizedProfit = 0;                           // Sum of closed-trade profit for the current broker day
int lastProfitResetDay = -1;                              // Day-of-year dailyRealizedProfit was last reset on

// On-Chart Dashboard P/L Reset - lets the "Reset P/L Stats" chart button zero
// the dashboard's displayed stats without touching real trade history
datetime statsResetTime = 0;                              // 0 = count from account inception (default)
string   statsResetGVName;                                // set in OnInit; unique per symbol+magic
...
int OnInit()
{
    // On-chart dashboard P/L reset: restores any previously-set reset point
    // from a terminal-level Global Variable, so it survives EA restarts.
    statsResetGVName = "NyaoDash_StatsReset_" + _Symbol + "_" + IntegerToString(MagicNumber);
    if(GlobalVariableCheck(statsResetGVName))
        statsResetTime = (datetime)GlobalVariableGet(statsResetGVName);
...
{
    // On-chart dashboard: "Reset P/L Stats" button. Doesn't touch real trade
    // history - just moves statsResetTime forward so the dashboard's stats
    // only count deals from this point on, and persists it as a Global
    // Variable so it survives EA restarts/recompiles until reset again.
    if(id == CHARTEVENT_OBJECT_CLICK && sparam == "NyaoDash_ResetBtn")
    {
        statsResetTime = TimeCurrent();
        GlobalVariableSet(statsResetGVName, (double)statsResetTime);
        ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_STATE, false); // un-press the button
        UpdateDashboard();
        LogPrint("[DASHBOARD] P/L stats reset at ", TimeToString(statsResetTime));
        return;
    }
                        ...
                        const MqlTradeResult &result)
{
    ...
    if(dealEntry == DEAL_ENTRY_IN)
    {
        ...
        // Trade Copier: relay this limit-order fill to clients, unless today's copy-profit limit is hit
        if(!CopyLimitReached())
        {
            double fillVolume = PositionSelectByTicket(posID) ? PositionGetDouble(POSITION_VOLUME)
                                                                : HistoryDealGetDouble(dealTicket, DEAL_VOLUME);
            double fillSL = PositionSelectByTicket(posID) ? PositionGetDouble(POSITION_SL) : 0;
            double fillTP = PositionSelectByTicket(posID) ? PositionGetDouble(POSITION_TP) : 0;
            BroadcastSignal("OPEN", posID, ptype, fillVolume, fillSL, fillTP, entryPrice, posComment);
        }
...
{   
    ...
    {   
        if(IsSpikeExhaustionBlocked(adjustedScore))
         return 0;
            ...
            if(managedPositions[posIndex].partialCloseLevel == 0 && signalRatio <= 0.75)
            {
                ...
                if(closeVol >= minVol && remaining >= minVol)
                {
                    if(PartialClosePosition(ticket, closeVol))
                    {
                        ...
                        // Trade Copier: mirror this partial close to clients. Not gated by
                        // CopyLimitReached() - it's managing an existing copied position,
                        // not opening a new one.
                        BroadcastSignal("PARTIAL_CLOSE", ticket, posType, closeVol, 0, 0, 0, "");
            ...
            else if(managedPositions[posIndex].partialCloseLevel == 1 && signalRatio <= 0.50)
            {
                ...
                if(closeVol >= minVol && remaining >= minVol)
                {
                    if(PartialClosePosition(ticket, closeVol))
                    {
                        ...
                        // Trade Copier: mirror this partial close to clients (see L1 note above)
                        BroadcastSignal("PARTIAL_CLOSE", ticket, posType, closeVol, 0, 0, 0, "");
...
void ProcessClosedPosition(ulong closedTicket, double closedProfit)
{
    ...
    int closedIndex = GetManagedPositionIndex(closedTicket);
    if(closedIndex == -1) return;

    // Trade Copier: always track daily realized profit and relay the close to
    // clients, regardless of copier state - not gated by CopyLimitReached(),
    // since already-copied clients must still be told to close out.
    UpdateDailyCopyProfitTracking(closedProfit);
    BroadcastSignal("CLOSE", closedTicket, managedPositions[closedIndex].type, 0, 0, 0, 0, "");
...
{   
    ...
    if(orderResult)
    {
        ...
        {   
            ...
            // Trade Copier: relay this new open to clients, unless today's copy-profit limit is hit
            if(!CopyLimitReached())
                BroadcastSignal("OPEN", result.order, posType, currentLot, request.sl, request.tp, price, request.comment);
...
ulong OpenChainHedge(ulong chainId, ENUM_POSITION_TYPE prevType, double hedgeLot, int newLevel, double anchorLoss, int cycleNum)
{
    ...
    if(orderResult && result.retcode == TRADE_RETCODE_DONE)
    {
        ...
        // Trade Copier: relay this hedge leg to clients, unless today's copy-profit limit is hit
        if(!CopyLimitReached())
            BroadcastSignal("OPEN", result.order, hedgePosType, hedgeLot, request.sl, request.tp, price, request.comment);
...
bool ModifyPosition(ulong ticket, double newSL, double newTP)
{
    ...
    // Trade Copier: relay every real SL/TP change (trailing stop included - every
    // subsystem that adjusts SL/TP goes through this one function) to clients.
    // Not gated by CopyLimitReached() - already-copied positions must keep getting
    // managed even after the daily copy limit stops new opens.
    BroadcastSignal("MODIFY", ticket, (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE),
                     PositionGetDouble(POSITION_VOLUME), newSL, newTP, 0, "");
...
void GetTradeStats(TradeStats& daily, TradeStats& allTime) 
{
    ...
    // statsResetTime is 0 (account inception) unless the "Reset P/L Stats"
    // chart dashboard button has been clicked, which moves this forward.
    if(HistorySelect(statsResetTime, now)) {
...
// +------------------------------------------------------------------+
// | Spike Exhaustion Block                                            |
// | "If one of the last 1-2 candles was an oversized sell or buy      |
// |  candle AND the score is very high (>=8), stay out."              |
// +------------------------------------------------------------------+
bool IsSpikeExhaustionBlocked(double score)
{
    if(!EnableSpikeExhaustionBlock) return false;
    if(score < SpikeExhaustionMinScore) return false;

    double bufATR[];
    if(CopyBuffer(atrSignalHandle, 0, 1, 1, bufATR) < 1) return false; // can't judge, don't block

    double atr = bufATR[0];
    if(atr <= 0) return false;

    int n = MathMax(1, SpikeExhaustionLookbackCandles);
    MqlRates rates[];
    ArraySetAsSeries(rates, true);
    if(CopyRates(_Symbol, _Period, 1, n, rates) < n) return false; // can't judge, don't block

    for(int i = 0; i < n; i++)
    {
        double range = rates[i].high - rates[i].low;
        if(range >= SpikeExhaustionRangeATR * atr)
        {
            LogPrint("[SPIKE EXHAUSTION] Blocked: candle ", i + 1, " back has range ",
                     DoubleToString(range, _Digits), " (>= ", DoubleToString(SpikeExhaustionRangeATR, 2),
                     "x ATR of ", DoubleToString(atr, _Digits), ") while score is ",
                     DoubleToString(score, 2), " >= ", DoubleToString(SpikeExhaustionMinScore, 2));
            return true;
        }
    }
    return false;
...
// +------------------------------------------------------------------+
// | Trade Copier - broadcast a signal to the Copier Server            |
// | Same WebRequest shape as SendDiscordAlert below, posting to       |
// | POST /api/admin/signal. A failed/slow post only logs a warning -  |
// | it must never be allowed to affect this EA's own order execution. |
// +------------------------------------------------------------------+
bool BroadcastSignal(string action, ulong ticket, ENUM_POSITION_TYPE type, double volume,
                      double sl, double tp, double price, string comment)
{
    if(!EnableTradeCopier || CopierServerURL == "" || CopierAdminKey == "") return false;

    string typeStr = (type == POSITION_TYPE_BUY) ? "BUY" : "SELL";
    string commentEscaped = comment;
    StringReplace(commentEscaped, "\\", "\\\\");
    StringReplace(commentEscaped, "\"", "\\\"");
    ...
    json += "\"symbol\":\"" + _Symbol + "\",";
    json += "\"type\":\"" + typeStr + "\",";
    json += "\"admin_ticket\":" + IntegerToString((long)ticket) + ",";
    ...
    json += "\"comment\":\"" + commentEscaped + "\"";
    json += "}";

    char post[];
    char result[];
    string headers = "Content-Type: application/json\r\nX-Admin-Key: " + CopierAdminKey + "\r\n";
    ...
    string url = CopierServerURL + "/api/admin/signal";
    int res = WebRequest("POST", url, headers, timeout, post, result, resultHeaders);

    if(res == 200)
    {
        LogPrint("[COPIER] Broadcast ", action, " for ticket ", ticket, " OK");
        return true;
    }
    else
    {
        LogPrint("[COPIER] Broadcast ", action, " for ticket ", ticket, " FAILED. HTTP: ", res,
...
// +------------------------------------------------------------------+
// | Trade Copier - roll dailyRealizedProfit over at the broker's day  |
// | boundary, then add this trade's realized profit. Called from     |
// | ProcessClosedPosition for every closed trade, regardless of      |
// | copier state, so the running total stays accurate.               |
// +------------------------------------------------------------------+
void UpdateDailyCopyProfitTracking(double closedProfit)
{
    MqlDateTime dt;
    TimeToStruct(TimeCurrent(), dt);

    if(dt.day_of_year != lastProfitResetDay)
    {
        dailyRealizedProfit = 0;
        lastProfitResetDay = dt.day_of_year;
    }

    dailyRealizedProfit += closedProfit;
}

// +------------------------------------------------------------------+
// | Trade Copier - true once today's realized profit has reached the |
// | configured limit. Only gates NEW-open broadcasts (see call       |
// | sites) - MODIFY/CLOSE broadcasts for already-copied positions    |
// | are never gated, so clients can still be managed out safely.     |
// +------------------------------------------------------------------+
bool CopyLimitReached()
{
    return DailyProfitLimitToStopCopying > 0 && dailyRealizedProfit >= DailyProfitLimitToStopCopying;
}
...
void UpdateDashboard()
{
    ...
    // Reset button: doesn't touch real trade history, just moves statsResetTime
    // forward so the dashboard's stats only count deals from this point on.
    ObjectCreate(0, "NyaoDash_ResetBtn", OBJ_BUTTON, 0, 0, 0);
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_CORNER, CORNER_LEFT_UPPER);
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_XDISTANCE, startX);
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_YDISTANCE, currentY);
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_XSIZE, 150);
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_YSIZE, 22);
    ObjectSetString(0, "NyaoDash_ResetBtn", OBJPROP_TEXT, "Reset P/L Stats");
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_COLOR, clrWhite);
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_BGCOLOR, clrDarkSlateGray);
    ObjectSetInteger(0, "NyaoDash_ResetBtn", OBJPROP_SELECTABLE, false);
}
// +------------------------------------------------------------------+